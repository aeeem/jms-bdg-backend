...
import {
  getCustomerDebtService,
  getCustomerDepositService
...
export const createTransactionService = async (
  ...
  try {
    ...
    const customerDeposit = customer
      ? ( await getCustomerDepositService( customer.id ) ).total_deposit
      : 0
    ...
    const transactionDetails = payload.detail.map( transactionDetail => {
      const stock = stocks.find(
        product => product.id === transactionDetail.stock_id
      )
    ...
    const stockSync = await Promise.all(
      payload.detail.map( async detail => {
        ...
        if ( payload.id_transaction ) {
          ...
          const findTransactionDetail = transactionDetail.find(
            item => item.stock_id === detail.stock_id
          )
        ...
        const stockHelper = await stockDeductor(
          detail.stock_id,
          detail.amount,
          detail.box,
          isPendingUpdate,
        ...
        deposit_balance: transactionProcess.transaction.customer
          ? (
              await getCustomerDepositService(
                transactionProcess.transaction?.customer?.id
              )
            ).total_deposit
          : 0,
...
export const updatePendingTransactionService = async (
  transaction_id: string,
  items: TransactionDetailRequestParameter[]
) => {
  const queryRunner = db.queryRunner()
  try {
    await queryRunner.startTransaction()
    ...
    const transactionDetailsIds = transaction.transactionDetails.map(
      detail => detail.stock_id
    )
    ...
    const updateData = await Promise.all(
      transaction.transactionDetails
        ...
        .map( async transactionDetail => {
          const masterStock = await Stock.findOneOrFail(
            transactionDetail.stock_id
          )
          const payload = items.find(
            item => item.stock_id === transactionDetail.stock_id
          )
          ...
          if ( payload.amount < + transactionDetail.amount ) {
            if ( payload.box ) {
              ...
              stockGudang.amount =
                Number( transactionDetail.amount ) - Number( payload.amount )
              stockGudang.code =
                E_GUDANG_CODE_KEY.GUD_ADD_BRG_PENDING_TRANSAKSI
            } else {
              masterStock.stock_toko =
                Number( masterStock.stock_toko ) +
                ( Number( transactionDetail.amount ) - Number( payload.amount ) )
...
export const deletePendingTransactionService = async ( id: string ) => {
  const transaction = await Transaction.findOneOrFail( id, { relations: ['transactionDetails'] } )
  if ( !transaction ) throw E_ERROR.TRANSACTION_NOT_FOUND
...
export const deletePendingTransactionItemService = async (
  payload: DeleteTransactionItemRequestParameter
) => {
...
export const updateTransactionService = async (
  id: string,
  payload: TransactionUpdateRequestParameter
) => {
  const queryRunner = db.queryRunner()
  try {
    await queryRunner.startTransaction()
    ...
    transaction.transactionDetails.map( detail => {
      const pd = payload.detail.find(
        detailPayload => detailPayload.id === detail.id
      )
      if ( pd ) {
